...
const gameResultData = ref<any>(null)
...
async function syncGameState() {
  ...
  try {
    ...
    if (resp.ok) {
      ...
      const userLobby = data.players?.find((p: any) => p.chat_id === currentUser && p.is_active);
      if (userLobby) lobbyId = userLobby.lobby_id;
    }
  } catch (e) { }
  if (!lobbyId) return;
  try {
    const playerStatusResp = await fetch(`/api/game/player-status?chat_id=${authStore.user?.chat_id}&lobby_id=${lobbyId}`);
...
onMounted(async () => {
  ...
  socketService.onGameResult(async (data: any) => {
    gameResultData.value = data
    let playerStatus = getCurrentPlayerStatus(data)
    if (!playerStatus && authStore.user?.chat_id && currentGameSession.value) {
      try {
        const resp = await fetch(`/api/game/player-status?chat_id=${authStore.user.chat_id}&lobby_id=${currentGameSession.value.lobby_id}`)
        if (resp.ok) {
    ...
    // Если игрок проиграл — показываем экран eliminated
    if (playerStatus === 'eliminated' || playerStatus === 'quit') {
      if (currentState.value !== 'eliminated') {
    ...
    // Не переключаем состояние если уже в gameover
    if (currentState.value === 'gameover') return;
  ...
  socketService.onPlayersEliminated((data: any) => {
    const currentUserId = authStore.user?.chat_id
  ...
  socketService.onPlayerStatusUpdate((data: any) => {
    ...
    if (currentUserId && data.statuses) {
      const currentUserStatus = data.statuses.find((s: any) => s.chat_id === currentUserId)
  ...
  try {
    if (localStorage.getItem('user')) {
      ...
      if (storedUser.chat_id) {
        await authStore.fetchUserData(storedUser.chat_id)
...
const handlePlayAgain = async () => {
  if (authStore.isAuthenticated && authStore.user && !authStore.user.is_admin) {
    try {
...
const handleSpectateGame = () => {
  // Player chose to spectate game
  switchState('gamewaiting')
}
...
function getCurrentPlayerStatus(resultObj?: any) {
  const result = resultObj || gameResultData.value
  if (!authStore.user?.chat_id || !result) return null;
  const stat = result.player_statistics?.find((s: any) => s.chat_id === authStore.user.chat_id)
  return stat ? stat.status : null;
}

// Удаляю watcher, чтобы не было автоматического возврата в lobby для проигравших
...
<template>
  ...
  <div v-else id="app">
    ...
    <div class="game-container">
      ...
      <div style="position: fixed; top: 10px; left: 10px; background: rgba(0,0,0,0.8); color: white; padding: 10px; border-radius: 5px; z-index: 9999; font-size: 12px;">
        <div>Current State: {{ currentState }}</div>
        <div>Auth: {{ authStore.isAuthenticated }}</div>
        <div>User: {{ authStore.user?.chat_id }}</div>
        <div>Game Result: {{ gameResultData ? 'Has data' : 'No data' }}</div>
      </div>
      ...
      <!-- Fallback для неизвестного состояния (всегда видимый) -->
      <div v-if="!['auth', 'lobby', 'waiting', 'game', 'choice', 'eliminated', 'admin', 'leaderboard', 'gameover', 'observer'].includes(currentState)" 
           style="text-align: center; color: white; padding: 20px; background: rgba(0,0,0,0.9); border-radius: 10px; margin: 20px;">
        <h2>Неизвестное состояние: "{{ currentState }}"</h2>
        <p>Это может быть ошибка. Попробуйте вернуться в лобби.</p>
        <button @click="switchState('lobby')" style="margin: 10px; padding: 10px 20px; background: #646cff; color: white; border: none; border-radius: 5px; cursor: pointer;">
          Вернуться в лобби
        </button>
      </div>
