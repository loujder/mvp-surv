...
const deductCoin = async (): Promise<boolean> => {
  ...
  try {
    const response = await fetch('http://localhost:5000/api/coins/deduct', {
      method: 'POST',
      headers: {
...
const loadBalance = async (): Promise<void> => {
  ...
  try {
    const response = await fetch(`http://localhost:5000/api/coins/balance?user_id=${authStore.user.user_id}`)
    if (response.ok) {
      const data = await response.json()
      if (currentPlayer.value) {
        currentPlayer.value.balance = data.balance
