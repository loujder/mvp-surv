...
const joinLobby = async (lobbyId: string) => {
  ...
  try {
    const response = await fetch('http://localhost:5000/api/lobby/join', {
      method: 'POST',
...
const leaveLobby = async () => {
  ...
  try {
    const response = await fetch('http://localhost:5000/api/lobby/leave', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        user_id: authStore.user.user_id
...
const loadLobbyPlayers = async () => {
  ...
  try {
    const apiUrl = isAdmin.value 
      ? `http://localhost:5000/api/admin/lobby/players?lobby_id=${currentLobbyId.value}`
      : 'http://localhost:5000/api/lobby/players'
...
const loadLobbyBank = async () => {
  try {
    const response = await fetch('http://localhost:5000/api/lobby/players')
    if (response.ok) {
      const data = await response.json()
...
const loadGameRounds = async () => {
  try {
    const response = await fetch('http://localhost:5000/api/game/status')
    if (response.ok) {
      const data = await response.json()
...
const joinGame = async () => {
  ...
  try {
    ...
    const deductResp = await fetch('http://localhost:5000/api/coins/deduct', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const response = await fetch('http://localhost:5000/api/lobby/ready', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        user_id: authStore.user.user_id
    ...
    if (response.ok) {
      ...
      try {
        const userResp = await fetch(`http://localhost:5000/api/user?user_id=${authStore.user.user_id}`)
        if (userResp.ok) {
...
const joinSpecificLobby = async (lobbyId: string) => {
  ...
  try {
    const response = await fetch(`http://localhost:5000/api/admin/lobby/${lobbyId}/join`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        user_id: authStore.user.user_id
...
const resetReadyStatus = async () => {
  ...
  try {
    const response = await fetch('http://localhost:5000/api/lobby/reset-ready', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        user_id: authStore.user.user_id
...
const checkRealReadyStatus = async () => {
  ...
  try {
    const response = await fetch(`http://localhost:5000/api/lobby/ready?user_id=${authStore.user.user_id}`)
    if (response.ok) {
      const data = await response.json()
