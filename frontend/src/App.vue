...
async function syncGameState() {
  let lobbyId = null;
  try {
    const resp = await fetch('http://localhost:5000/api/lobby/players');
    if (resp.ok) {
  ...
  if (!lobbyId) return;
  try {
    const playerStatusResp = await fetch(`http://localhost:5000/api/game/player-status?user_id=${authStore.user?.user_id}&lobby_id=${lobbyId}`);
  ...
  try {
    const statusResp = await fetch(`http://localhost:5000/api/game/status?lobby_id=${lobbyId}`);
...
onMounted(async () => {
  ...
  socketService.onGameResult(async (data: any) => {
    ...
    if (!playerStatus && authStore.user?.user_id && currentGameSession.value) {
      try {
        const resp = await fetch(`http://localhost:5000/api/game/player-status?user_id=${authStore.user.user_id}&lobby_id=${currentGameSession.value.lobby_id}`)
        if (resp.ok) {
...
const handlePlayAgain = async () => {
  if (authStore.isAuthenticated && authStore.user && authStore.user.user_id !== 'Loujder') {
    try {
      await fetch('http://localhost:5000/api/lobby/unready', {
        method: 'POST',
...
const handleExit = async () => {
  if (authStore.isAuthenticated && authStore.user && authStore.user.user_id !== 'Loujder') {
    try {
      ...
      await fetch('http://localhost:5000/api/lobby/leave', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ user_id: authStore.user.user_id })
      })
